...
args = Dict("num_depth" => 10, "batch_size" => 256)
...
begin
	histogram(data_1_tr[:], xlim=[-5:5])
	#histogram!(data_2_tr[:], xlims=[-5:5])
	#histogram!(data_3_tr[:], xlims=[-5:5])
	#histogram!(data_4_tr[:], xlims=[-5:5])
end

# ╔═╡ 2104873b-10b5-495e-89f2-d29a17f33ec4
# begin
# 	num_samples2 = size(data_1)[end] ÷ args["num_depth"];
# 	data_1_tr2 = reshape(data_1[:, :, 1:num_samples2 * args["num_depth"]], (24, 8, args["num_depth"], 1, num_samples));
# 	data_1_tr2 = (data_1_tr2 .- mean(data_1_tr2)) / std(data_1_tr2);
# 	0.0
# end
...
# ╔═╡ 9d8bff7d-c8b9-42da-9104-2445c7b776b7
begin
	histogram(data_1[:]) #, xlims=[-, 5.0])
	histogram!(data_1_tr[:])
	histogram!(data_1_tr2[:])
end
...
begin
	...
	cat_idx_1 = onecold(model_c(x1)[end-1:end, :])
	cat_idx_2 = onecold(model_c(x2)[end-1:end, :])
	cat_idx_3 = onecold(model_c(x3)[end-1:end, :])
	cat_idx_4 = onecold(model_c(x4)[end-1:end, :])
end
